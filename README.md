...
jj-agentic-switch update                # 自更新到 GitHub Releases latest
jj-agentic-switch -h
```

> 备份: `~/.config/jj-agentic-switch/{cc,cx}/auth-backup-<email>.json` (0600). 无法识别的 cc 当前凭据另存 `auth-recovery-<fingerprint>.json`. 含 refresh token, NEVER 提交 git.

## 架构

Bun + TypeScript 编译为 macOS arm64 + x64 单文件 binary. 凭据切换直接读写 macOS Keychain (`security`) + 备份至 `~/.config/jj-agentic-switch`; cc 切换同步清 `~/.claude.json` 身份缓存. 查看状态 / 切换时自动捕获当前活跃账号入备份库 (token 未变则跳过, 首次收录时提示), 用户无需手动 `backup`. 安装 / 自更新按 arch 取对应 binary, 附 sha256 校验. 发布走 GitHub Actions tag push 自动编译上传 (见 [workflow.md](./workflow.md)).
